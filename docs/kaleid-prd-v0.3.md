...
变更日志:
- v0.3 PATCH（2026-05-22，ByAnDa msg=4a0038ef）：login/logout/exit 改为 REPL slash 命令（/login //logout //exit + /help），移除顶层 CLI 子命令 kaleid login/logout。落为 spec-008，派 Multica 实施。F4 已更新。
...
- **REPL slash 命令（spec-008，post-V1 迭代）**：login / logout / 退出改为 REPL 内 slash 命令 —— `/login`（OAuth 登录）/ `/logout`（删本地凭证）/ `/exit`（退出，替代输入 `exit`/`quit`）/ `/help`（列命令）。输入以 `/` 开头视为命令、否则当 prompt。**移除顶层 CLI 子命令 `kaleid login`/`kaleid logout`**。详见 `kaleid spec-008 slash 命令.md`。
