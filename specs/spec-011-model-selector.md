...
### 4. header 显示当前模型（右上角）
- spec-010 的 Header（`src/tui/components/Header.tsx`）：左侧 `kaleid`，**右上角显示当前模型名**（如 `model: gpt-5.5`）。
- 切换模型后 header 实时更新。
- 防闪：模型名变更只 patch header 那一行（依赖 spec-010 的 diff renderer）。
...
7. WHEN header 显示 THEN 始终可见当前模型名（含启动初值 gpt-5.5 或 --model 指定值）。
8. 防闪：选择器开关/切换只 patch 变化行，不整屏闪（沿用 spec-010 diff renderer）。
9. clean-room；测试用 fake；typecheck/test/build/pack 全绿；`npm pack` 仍只 README+dist+package.json。

## 涉及文件（修改/新增）
- `src/provider/models.ts`（新建，建议）— `AVAILABLE_MODELS` 常量 + 默认模型，单一来源
- `src/tui/commands.ts` — slash 表加 `/model`
- `src/tui/components/ModelSelector.tsx`（新建）— 选择器列表 + 高亮 + 键盘交互
- `src/tui/app.tsx` — `/model` 进选择器子状态、应用所选、对话区提示
- `src/tui/components/Header.tsx` — 右上角当前模型名
- provider/loop — 当前 model 作为可变会话状态（runTurn 用当前值）
- `test/` — /model 列表/高亮/应用/Esc/header 更新 覆盖
